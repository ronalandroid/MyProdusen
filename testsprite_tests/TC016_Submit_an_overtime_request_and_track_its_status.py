...
async def run_test():
    ...
    try:
        ...
        # -> Fill the login form with admin@myprodusen.com and Admin@Produsen2026, then submit the form.
        ...
        # -> Close the install popup and scroll the dashboard/navigation further to reveal the 'Lembur' (overtime) menu item or link so it can be opened.
        # button aria-label="Tutup popup install"
        elem = page.locator("xpath=/html/body/aside/button").nth(0)
        await elem.wait_for(state="visible", timeout=10000)
        await elem.click()
        
        # -> Open the overtime (Lembur) page so an overtime request can be created.
        ...
        assert await page.locator("xpath=//*[contains(., 'Pengajuan Lembur')]").nth(0).is_visible(), "The overtime request should be displayed in the Pengajuan Lembur list after creation"
        assert await page.locator("xpath=//*[contains(., 'Menunggu Persetujuan')]").nth(0).is_visible(), "The overtime request status should be visible as Menunggu Persetujuan after submission"
