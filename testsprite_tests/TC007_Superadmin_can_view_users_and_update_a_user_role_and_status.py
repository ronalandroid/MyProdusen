...
async def run_test():
    ...
    try:
        ...
        # -> Close the Install popup if present, then open the user management view (Pengguna).
        ...
        # -> Open the role dropdown for the first user in the list so the role options are revealed (click the select control at index 1259).
        # "Superadmin Karyawan"
        elem = page.locator("xpath=/html/body/div[2]/div/main/main/section[2]/div[3]/article/div[2]/label/select").nth(0)
        await elem.wait_for(state="visible", timeout=10000)
        await elem.click()
        
        # -> Set the first user's role to 'Karyawan' using the select (index 1259), then click the 'Nonaktifkan' button for that user (index 1265). After those actions, verify the role and active status changed in the UI.
        # button "Nonaktifkan"
        elem = page.locator("xpath=/html/body/div[2]/div/main/main/section[2]/div[4]/article/div[2]/button").nth(0)
        await elem.wait_for(state="visible", timeout=10000)
        await elem.click()
