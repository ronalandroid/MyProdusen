...
async def run_test():
    ...
    try:
        ...
        # -> Fill in the login credentials for the employee/admin account (email and password) and submit the login form.
        ...
        # -> Open the leave request screen by clicking the 'Cuti' link in the left navigation.
        # link "Cuti"
        elem = page.locator("xpath=/html/body/div[2]/div/nav/nav/a[8]").nth(0)
        await elem.wait_for(state="visible", timeout=10000)
        await elem.click()
        
        # -> Close the install popup if present, then open the 'create new leave' form by clicking the add/new (+) button.
        ...
        # -> Fill Tanggal Mulai with a date overlapping an existing leave (use 2026-06-25), fill Tanggal Selesai (2026-06-26), add a reason, submit the request, then check for an overlap validation error on the page.
        # date input
        ...
        await elem.wait_for(state="visible", timeout=10000)
        await elem.fill("2026-06-26")
        
        # -> Fill Tanggal Mulai with a date overlapping an existing leave (use 2026-06-25), fill Tanggal Selesai (2026-06-26), add a reason, submit the request, then check for an overlap validation error on the page.
        # placeholder="Jelaskan alasan pengajuan..."
        elem = page.locator("xpath=/html/body/div[2]/div/main/div/div[4]/div[2]/div[2]/form/div[4]/textarea").nth(0)
        await elem.wait_for(state="visible", timeout=10000)
        await elem.fill("Uji tumpang tindih: pengajuan menumpuk dengan cuti 25-26 Jun 2026")
