...
async def run_test():
    ...
    try:
        ...
        # -> Click the 'Masuk Sistem' (login) button to open the login page.
        # link "Masuk Sistem"
        ...
        # -> Open the 'Akun' (Account) sidebar menu to locate the Employees (Karyawan) or Employee management page.
        # link "Akun"
        elem = page.locator("xpath=/html/body/div[2]/div/nav/nav/a[16]").nth(0)
        await elem.wait_for(state="visible", timeout=10000)
        await elem.click()
        
        # -> Navigate to the employee management page so the create-employee flow can be started (use direct navigation since no visible link was found).
        ...
        # -> Open the 'Tambah Karyawan' (Add employee) form by clicking the add button so the create-employee form fields are visible.
        # button
        elem = page.locator("xpath=/html/body/div[2]/div/main/div/button").nth(0)
        await elem.wait_for(state="visible", timeout=10000)
        await elem.click()
        
        # -> Fill the employee form (Nama Lengkap, Email, Username, Password), close the Install App popup if it blocks, save the new employee, then verify the new employee appears in the employees table by searching for the email.
        # text input
        elem = page.locator("xpath=/html/body/div[2]/div/main/div/div[4]/div[2]/div[2]/form/div[2]/div/input").nth(0)
        await elem.wait_for(state="visible", timeout=10000)
        await elem.fill("E2E Employee 9001")
        
        # -> Fill the employee form (Nama Lengkap, Email, Username, Password), close the Install App popup if it blocks, save the new employee, then verify the new employee appears in the employees table by searching for the email.
        # email input
        elem = page.locator("xpath=/html/body/div[2]/div/main/div/div[4]/div[2]/div[2]/form/div[3]/div/input").nth(0)
        await elem.wait_for(state="visible", timeout=10000)
        await elem.fill("e2e9001@example.com")
        ...
        # -> Click the 'Simpan' button to save the new employee, wait for the UI to update, then search the page for 'e2e9001@example.com' to verify the new record appears in the employees table.
        # button "Simpan"
