...
async def run_test():
    ...
    try:
        ...
        # -> Click the 'Masuk Sistem' link to open the login page.
        # link "Masuk Sistem"
        elem = page.locator("xpath=/html/body/div[2]/div/nav/div[2]/a").nth(0)
        await elem.wait_for(state="visible", timeout=10000)
        await elem.click()
        
        # -> Fill superadmin credentials with provided account and submit the login form.
        ...
        # -> Open the Employees list (navigate to employee listing) by clicking the 'Total Karyawan' card or equivalent link. Then wait for the employees page to load.
        # link "Total Karyawan 14 3 Superadmin · 9 Karya..."
        elem = page.locator("xpath=/html/body/div[2]/div/main/div/section/div[2]/a").nth(0)
        await elem.wait_for(state="visible", timeout=10000)
        await elem.click()
        
        # -> Close the 'Install MyProdusen' popup so the employees list is fully accessible, then perform search and open the status dropdown to apply a filter.
        ...
        await elem.wait_for(state="visible", timeout=10000)
        await elem.fill("Superadmin Local")
        
        # -> Close the 'Install MyProdusen' popup so the employees list is fully accessible, then perform search and open the status dropdown to apply a filter.
        # "Semua Status Aktif Nonaktif Cuti"
        elem = page.locator("xpath=/html/body/div[2]/div/main/div/div[2]/select").nth(0)
        await elem.wait_for(state="visible", timeout=10000)
        await elem.click()
        
        # -> Click the employee's view/edit button to open the detail view for 'Superadmin Local' and then verify the detail information is displayed.
        # button
