...
async def run_test():
    ...
    try:
        ...
        # -> Open the login page by clicking 'Masuk Sistem'.
        # link "Masuk Sistem"
        ...
        # -> Click the 'Laporan' (Reports) link in the main navigation to open the reports index.
        # link "Laporan"
        elem = page.locator("xpath=/html/body/div[2]/div/nav/nav/a[10]").nth(0)
        await elem.wait_for(state="visible", timeout=10000)
        await elem.click()
        
        # -> Click the 'Hanya terlambat' checkbox to filter only late records, then click 'Muat Ulang' to apply the filter and wait for the results to load.
        # checkbox input
        elem = page.locator("xpath=/html/body/div[2]/div/main/div/section/div[2]/label/input").nth(0)
        await elem.wait_for(state="visible", timeout=10000)
        await elem.click()
