...
describe('Role Navigation Policy', () => {
  ...
  it('EMPLOYEE sees personal navigation only', () => {
    ...
    expect(nav.map((item) => item.key)).not.toContain('overtime');
    expect(nav.map((item) => item.key)).not.toContain('documents');
    expect(nav.map((item) => item.key)).toContain('payroll');
  ...
  it('SUPERADMIN primary tabs match the design', () => {
    ...
    expect(keys).toEqual(['dashboard', 'employees', 'kpi', 'payroll', 'profile']);
  });
